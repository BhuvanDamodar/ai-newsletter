...
class EmailDeliverer:
    ...
    def render_email_html(self, user: User, articles: list[Content]) -> str:
        """Parses the LLM JSON strings, generates signed feedback URLs, and renders the final HTML string."""
        ...
        for article in articles:
            ...
            # Generate signed feedback tokens for this user+article pair
            try:
                like_token = generate_feedback_token(user.id, article.id, rating=1)
                dislike_token = generate_feedback_token(user.id, article.id, rating=-1)
                like_url = f"{FRONTEND_URL}/feedback?token={like_token}"
                dislike_url = f"{FRONTEND_URL}/feedback?token={dislike_token}"
            except Exception as e:
                logger.error(f"Failed to generate feedback tokens for article {article.id}: {e}")
                like_url = ""
                dislike_url = ""
